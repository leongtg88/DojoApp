...
export function AdminInstructorRoles({ candidates }: AdminInstructorRolesProps) {
  ...
  return (
    <section className="mt-3 rounded-lg border border-neutral-800 bg-[#161b22] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-2 rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-1.5 text-xs font-semibold text-neutral-300">
          <ShieldCheck aria-hidden="true" className="size-4 text-cyan-400" />{instructorCount} instructores
        </span>
      </div>
      ...
      {candidates.length === 0 ? (
        <p className="mt-4 flex items-center gap-2 rounded-md border border-dashed border-neutral-700 bg-[#0d1117] p-4 text-sm text-neutral-400">
          <Users aria-hidden="true" className="size-4" />Aún no hay alumnos con cuenta activa para habilitar como instructores.
        </p>
      ) : filtered.length === 0 ? (
        <p className="mt-4 rounded-md border border-dashed border-neutral-700 bg-[#0d1117] p-4 text-sm text-neutral-400">Sin resultados para “{query}”.</p>
      ) : (
        <ul className="mt-4 max-h-80 divide-y divide-neutral-800 overflow-y-auto rounded-md border border-neutral-800 bg-[#0d1117]">
          {filtered.map((candidate) => (
            <li className="flex items-center justify-between gap-3 px-4 py-3" key={candidate.studentId}>
              <div className="min-w-0">
                <p className="flex items-center gap-2 truncate text-sm font-semibold text-white">
                  ...
                  {candidate.isInstructor && (
                    <span className="inline-flex items-center gap-1 rounded-full border border-cyan-900/40 bg-cyan-950/60 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-cyan-300">
              ...
              <button
                className={`inline-flex shrink-0 items-center gap-1.5 rounded-md border px-3 py-2 text-xs font-semibold transition-colors disabled:opacity-60 ${candidate.isInstructor ? 'border-red-500/40 bg-red-500/10 text-red-300 hover:bg-red-500/20' : 'border-cyan-500/40 bg-cyan-950/30 text-cyan-200 hover:bg-cyan-900/50'}`}
      ...
      <p className="mt-3 text-[10px] text-neutral-500">El nuevo rol se activa cuando la persona vuelve a iniciar sesión.</p>
    </section>
  )
}
