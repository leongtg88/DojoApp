                ...
                <ul className="mt-7 divide-y divide-edge rounded-lg border border-edge bg-surface-2">
                    {enrollments.map((enrollment) => (
                        <li className="px-5 py-4" key={enrollment.id}>
                            <p className="text-sm font-semibold text-ink">{enrollment.applicantName ?? 'Nombre pendiente'}</p>
                            <p className="mt-1 text-sm text-ink-2">{enrollment.contactEmail} · {enrollment.contactPhone ?? 'Sin teléfono'}</p>
                            <p className="mt-2 text-xs text-ink-3">{enrollment.interest ?? 'Sin programa'} · {enrollment.schedule ?? 'Sin horario'} · {enrollment.createdAtLabel ?? enrollment.createdAt}</p>
                            {enrollment.applicants.length > 0 && <p className="mt-1 text-xs font-semibold text-accent">{enrollment.applicants.length} aspirante{enrollment.applicants.length === 1 ? '' : 's'} pendiente{enrollment.applicants.length === 1 ? '' : 's'}</p>}
                            <div className="mt-3 flex flex-wrap items-center gap-2">
                                <button className="rounded-md border border-cyan-500/40 px-3 py-2 text-sm font-semibold text-accent-text hover:bg-cyan-500/10 disabled:opacity-60" disabled={isSaving} onClick={() => openConversion(enrollment)} type="button">
                                    Convertir en alumno
                                </button>
                                <button className="inline-flex items-center gap-1.5 rounded-md border border-red-500/40 px-3 py-2 text-sm font-semibold text-danger-text hover:bg-red-500/10 disabled:opacity-60" disabled={isSaving} onClick={() => deleteEnrollment(enrollment)} type="button"><Trash2 aria-hidden="true" className="size-4" />Eliminar</button>
                            </div>
                        </li>
                    ))}
                </ul>
...
<select className="mt-1.5 block w-full rounded-md border border-edge-strong bg-surface-1 px-3 py-2 text-sm text-ink" id="gender" onChange={(event) => setGender(event.target.value as 'FEMALE' | 'MALE' | '')} value={gender}>
                        ...
                        <div className="mt-5 flex gap-3">
                            ...
                            <button className="rounded-md border border-edge-strong px-4 py-2.5 text-sm font-semibold text-ink-2 hover:bg-surface-3" onClick={() => setSelectedEnrollment(null)} type="button">Cancelar</button>
                        </div>
                    </form>
                </div>
            )}
