        ...
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
            <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-lg border border-edge bg-surface-2 shadow-xl" onClick={(event) => event.stopPropagation()}>
                <div className="flex items-center justify-between border-b border-edge px-5 py-4">
                    <div>
                        <h3 className="font-display text-lg font-bold text-ink">Plan, horarios y beca</h3>
                        <p className="text-xs text-ink-3">{studentName}</p>
                    </div>
                ...
                <div className="flex-1 space-y-5 overflow-y-auto px-5 py-4">
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <fieldset>
                                <legend className="text-xs font-bold uppercase tracking-wide text-ink-3">Plan de mensualidad</legend>
                                <select
                                    value={planId}
                            ...
                            <fieldset>
                                <legend className="text-xs font-bold uppercase tracking-wide text-ink-3">Horarios de referencia</legend>
                                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                                    {[...grouped.entries()].sort(([a], [b]) => a - b).map(([day, blocks]) => (
                                        <div key={day} className="rounded-md border border-edge bg-surface-1 p-3">
                                            <p className="mb-2 text-xs font-bold text-ink-2">{DAY_LABELS[day]}</p>
                                            <div className="space-y-1.5">
                                                {blocks.map((schedule) => {
                                                    const checked = scheduleIds.includes(schedule.id)
                                                    return (
                                                        <label key={schedule.id} className="flex items-center gap-2 text-sm text-ink">
                                                            <input
                                                                type="checkbox"
                                                                checked={checked}
                                                            ...
                                                            <span className="ml-auto text-xs text-ink-4">{schedule.startTime}</span>
                                                        </label>
                                                    )
                                                })}
                                            </div>
                                    ...
                                    {grouped.size === 0 && <p className="text-sm text-ink-4">No hay horarios activos.</p>}
                                </div>
                            </fieldset>

                            <fieldset>
                                <legend className="text-xs font-bold uppercase tracking-wide text-ink-3">Beca / competencia</legend>
                ...
                <div className="flex items-center justify-end gap-2.5 border-t border-edge px-5 py-3.5">
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-md border border-edge-strong bg-surface-1 px-4 py-2 text-xs font-semibold text-ink-2 hover:bg-surface-3"
                    >
                        Cancelar
                    </button>
                    <button
