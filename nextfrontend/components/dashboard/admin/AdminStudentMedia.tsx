	...
	return (
		<section className="mt-7 rounded-lg border border-neutral-800 bg-[#161b22] shadow-sm">
			<div className="flex items-center justify-between border-b border-neutral-800 px-5 py-4">
				<div>
					<p className="text-xs font-semibold uppercase tracking-wide text-cyan-400">Galería</p>
					<h2 className="mt-1 font-display text-lg font-bold text-white">Imágenes cargadas</h2>
				</div>
			</div>

			{items.length === 0 ? (
				<p className="flex items-center gap-2 px-5 py-8 text-sm text-neutral-400">
					<ImageOff aria-hidden="true" className="size-4 text-neutral-500" />
				...
				<div className="grid grid-cols-2 gap-3 p-5 sm:grid-cols-3 lg:grid-cols-4">
					{items.map((item, index) => (
						<button
							...
							className="group relative aspect-square overflow-hidden rounded-lg border border-neutral-800 bg-[#0d1117] transition-colors hover:border-cyan-500/40"
						>
							...
							<span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent px-2 py-1.5 text-left text-[11px] font-semibold text-white">{item.label}</span>
						</button>
					))}
				</div>
			)}
			...
			{activeItem && (
				<div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/90 p-4" onClick={() => setActiveIndex(null)}>
					<div className="absolute inset-x-4 top-4 flex items-center justify-between">
						<p className="text-xs font-semibold uppercase tracking-wide text-cyan-300">{activeItem.label} · {activeIndex! + 1} / {items.length}</p>
