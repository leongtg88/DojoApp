                ...
                {techniques.length > 0 && (
                    <div className="flex flex-col gap-3 border-b border-neutral-800 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
                        ...
                        <div className="flex flex-wrap items-center gap-2 overflow-x-auto pb-1">
                            <div className="flex shrink-0 items-center gap-1 rounded-md border border-neutral-700 bg-[#0d1117] p-1">
                                {([['YOUTH', 'Niños'], ['ADULT', 'Adultos']] as const).map(([value, label]) => (
                                    <button aria-pressed={programFilter === value} className={`rounded px-2.5 py-1 text-xs font-bold transition-colors ${programFilter === value ? 'bg-cyan-500 text-[#0d1117]' : 'text-neutral-400 hover:text-white'}`} key={value} onClick={() => setProgramFilter(value)} type="button">{label}</button>
                                ))}
                            </div>
                            <span aria-hidden="true" className="h-5 w-px shrink-0 bg-neutral-700" />
                            ...
                            return (
                                <li className="flex items-start justify-between gap-4 px-5 py-4" key={technique.id}>
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-xs text-neutral-400">{techniqueMetaLine(technique)}</p>
                                        ...
                                        {level ? (
                                            ...
                                            <p className="mt-2 inline-flex items-center gap-1.5 rounded-md border border-neutral-700 bg-[#0d1117] px-2 py-1 text-[11px] font-semibold text-cyan-300"><span aria-hidden="true" className="inline-block h-2.5 w-3.5 rounded-sm border border-white/30" style={{ backgroundColor: rank.beltColor ?? '#3f3f46' }} />{rank.name}</p>
                                        ) : null}
                                    </div>
...
function CategoryPickerDialog({ onClose, onSelect }: { onClose: () => void; onSelect: (category: TechniqueCategory) => void }) {
    const icons: Record<TechniqueCategory, React.ReactNode> = {
        KIHON: <Shield aria-hidden="true" className="size-6 text-cyan-400" />,
        KATA: <BookOpen aria-hidden="true" className="size-6 text-emerald-400" />,
        KUMITE: <Swords aria-hidden="true" className="size-6 text-red-400" />,
        ...
        <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog">
            <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl border border-neutral-800 bg-[#161616] p-6 shadow-2xl">
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-cyan-400">Nueva técnica</p>
                        <h3 className="mt-1 font-display text-lg font-bold text-white">¿Qué tipo de técnica vas a crear?</h3>
                    </div>
                ...
                <div className="mt-5 grid gap-3">
                    {TECHNIQUE_CATEGORIES.map((category) => (
                        <button className="flex items-start gap-3 rounded-lg border border-neutral-700 bg-[#0d1117] p-4 text-left transition-colors hover:border-cyan-500/50 hover:bg-cyan-500/5" key={category} onClick={() => onSelect(category)} type="button">
                            <span className="mt-0.5 shrink-0">{icons[category]}</span>
                            <span className="min-w-0">
                                <span className="block text-sm font-bold text-white">{TECHNIQUE_CATEGORY_LABELS[category]}</span>
                                <span className="mt-0.5 block text-xs text-neutral-400">{CATEGORY_DESCRIPTIONS[category]}</span>
                            </span>
                        </button>
                    ))}
                </div>
    ...
    return (
        <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog">
            <form className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl border border-neutral-800 bg-[#161616] p-6 shadow-2xl" onSubmit={onSubmit}>
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-cyan-400">{isNew ? `Nueva técnica · ${TECHNIQUE_CATEGORY_LABELS[category]}` : `Editar técnica · ${TECHNIQUE_CATEGORY_LABELS[category]}`}</p>
                        <h3 className="mt-1 font-display text-lg font-bold text-white">{isNew ? `Crear ${TECHNIQUE_CATEGORY_LABELS[category]}` : form.name}</h3>
                    </div>
                    <button aria-label="Cerrar" className="flex size-8 shrink-0 items-center justify-center rounded-full text-neutral-400 transition-colors hover:bg-neutral-800 hover:text-white" onClick={onClose} type="button"><X aria-hidden="true" className="size-5" /></button>
                </div>

                <div className="mt-5 grid gap-4">
                    <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-name">Nombre<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-name" onChange={(event) => set('name', event.target.value)} placeholder={category === 'KIHON' ? 'Age Uke' : category === 'KUMITE' ? 'Kihon Ippon Kumite - Jodan' : category === 'BUNKAI' ? 'Bunkai de Pinan Nidan - Secuencia 1' : 'Pinan Nidan'} required value={form.name} /></label>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-category">Tipo de técnica<select className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white" id="technique-category" onChange={(event) => changeCategory(event.target.value as TechniqueCategory)} value={category}>{TECHNIQUE_CATEGORIES.map((option) => <option key={option} value={option}>{TECHNIQUE_CATEGORY_LABELS[option]}</option>)}</select></label>
                    </div>
                    ...
                    {category === 'KIHON' && (
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-difficulty">Dificultad<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-difficulty" onChange={(event) => set('difficulty', event.target.value)} placeholder="Baja" value={form.difficulty} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-repetitions">N.º de repeticiones<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-repetitions" min="0" onChange={(event) => set('repetitionsCount', event.target.value)} placeholder="10" type="number" value={form.repetitionsCount} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-stance">Posición / Guardia<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-stance" onChange={(event) => set('stance', event.target.value)} placeholder="Zenkutsu Dachi" value={form.stance} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-level">Nivel<select className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white" id="technique-level" onChange={(event) => set('level', event.target.value)} value={form.level}><option value="">Sin especificar</option><option value="Jodan">Jodan</option><option value="Chudan">Chudan</option><option value="Gedan">Gedan</option></select></label>
                        </div>
                    )}

                    {category === 'KUMITE' && (
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-kumite-type">Tipo de kumite<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-kumite-type" onChange={(event) => set('kumiteType', event.target.value)} placeholder="Kihon Ippon, Jiyu Ippon, Shiai..." value={form.kumiteType} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-difficulty">Dificultad<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-difficulty" onChange={(event) => set('difficulty', event.target.value)} placeholder="Media" value={form.difficulty} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-movements">N.º de pasos / técnicas<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-movements" min="0" onChange={(event) => set('movementsCount', event.target.value)} type="number" value={form.movementsCount} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-distance">Distancia<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-distance" onChange={(event) => set('distance', event.target.value)} placeholder="Toma, Chika Ma, Ma..." value={form.distance} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-role">Rol<select className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white" id="technique-role" onChange={(event) => set('role', event.target.value)} value={form.role}><option value="">Sin especificar</option><option value="Aka">Aka (Rojo)</option><option value="Ao">Ao (Azul)</option><option value="Tori">Tori (Ejecutor)</option><option value="Uke">Uke (Receptor)</option></select></label>
                        </div>
                    )}

                    {category === 'BUNKAI' && (
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-application">Tipo de aplicación<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-application" onChange={(event) => set('applicationType', event.target.value)} placeholder="Defensa personal, Proyección..." value={form.applicationType} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-difficulty">Dificultad<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-difficulty" onChange={(event) => set('difficulty', event.target.value)} placeholder="Media" value={form.difficulty} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-movements">N.º de movimientos / secuencias<input className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-movements" min="0" onChange={(event) => set('movementsCount', event.target.value)} type="number" value={form.movementsCount} /></label>
                            <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-origin-kata">Kata de origen<select className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm text-white" id="technique-origin-kata" onChange={(event) => set('originKataId', event.target.value)} value={form.originKataId}><option value="">Sin kata de origen</option>{kataOptions.map((kata) => <option key={kata.id} value={kata.id}>{kata.name}</option>)}</select></label>
                        </div>
                    )}
                    ...
                    <label className="text-xs font-semibold text-neutral-300" htmlFor="technique-desc">Descripción y requisitos<textarea className="mt-1.5 w-full rounded-md border border-neutral-700 bg-[#0d1117] px-3 py-2 text-sm font-normal text-white outline-none placeholder:text-neutral-500 focus:border-cyan-500" id="technique-desc" onChange={(event) => set('description', event.target.value)} placeholder={category === 'KUMITE' ? 'El atacante avanza con Oi Zuki Jodan, el defensor retrocede con Age Uke y contraataca...' : 'Detalle técnico de la ejecución'} rows={3} value={form.description} /></label>
                </div>
