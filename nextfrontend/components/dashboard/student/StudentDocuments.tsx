...
import { ExternalLink, FileUp, FileWarning, LoaderCircle, Trash2 } from 'lucide-react'
...
export function StudentDocuments({ documents, studentId }: StudentDocumentsProps) {
    ...
    async function uploadDocument(event: React.FormEvent<HTMLFormElement>) {
        ...
        const response = await fetch('/api/dashboard/student/documents', { method: 'POST', headers: studentHeader, body: formData })
    ...
    async function openDocument(documentId: string) {
        ...
        const query = studentId ? `?documentId=${documentId}&estudiante=${encodeURIComponent(studentId)}` : `?documentId=${documentId}`
        const response = await fetch(`/api/dashboard/student/documents${query}`)
        const result = await response.json().catch(() => null)
        setOpeningId(null)
    ...
    async function deleteDocument(documentId: string) {
        const label = documentLabels[items.find((document) => document.id === documentId)?.type ?? 'OTHER']
        if (!window.confirm(`¿Eliminar ${label}? Esta acción no se puede deshacer.`)) return
        setError(null); setDeletingId(documentId)
        const query = studentId ? `?documentId=${documentId}&estudiante=${encodeURIComponent(studentId)}` : `?documentId=${documentId}`
        const response = await fetch(`/api/dashboard/student/documents${query}`, { method: 'DELETE', headers: studentHeader })
        setDeletingId(null)
        if (!response.ok) {
            const result = await response.json().catch(() => null)
        ...
        setItems((current) => current.filter((document) => document.id !== documentId))
    }
