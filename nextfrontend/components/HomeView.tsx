    ...
    <div className="space-y-0 min-h-screen bg-brand-bg text-[#dee2f0]">
      ...
      <section id="sucursal" className="py-15 max-w-7xl mx-auto px-8 sm:px-6 md:px-[50px] scroll-mt-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          <div id="contacto" className="lg:col-span-5 space-y-6 text-left backdrop-blur-xs scroll-mt-24">
            ...
            <div className="pt-2 hidden lg:block">
              <a href="https://www.google.com/maps/place/Karate+Do+Tosei+Gusoku+Dojo+Shito+Ryu+Inoue+Ha" target="_blank" rel="noopener noreferrer" className="hero-button glass-card-hover w-full sm:w-auto">
                ¿CÓMO LLEGAR AL DOJO? <Navigation className="w-4 h-4 shrink-0" />
              </a>
            </div>
          </div>
        ...
        <div className="pt-4 lg:hidden">
          <a href="https://www.google.com/maps/place/Karate+Do+Tosei+Gusoku+Dojo+Shito+Ryu+Inoue+Ha" target="_blank" rel="noopener noreferrer" className="hero-button glass-card-hover w-full">
            ¿CÓMO LLEGAR AL DOJO? <Navigation className="w-4 h-4 shrink-0" />
          </a>
        </div>
      </section>
      ...
      {/* Horarios */}
      <section id="horarios" className="md:py-20 px-8 md:px-[50px] bg-white border-y border-white/5 scroll-mt-24">
        <div className="max-w-7xl mx-auto space-y-1">
          <div className="text-left">
            ...
            <h2 className="text-3xl text-left sm:text-4xl font-extrabold font-display tracking-tight text-gray-700">Horarios de Entrenamiento</h2>
          </div>
      ...
      <section className="md:py-20 max-w-7xl mx-auto px-8 md:px-[50px] space-y-6 md:space-y-12">
        <div className="text-left space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold font-display tracking-tight text-gray-700">Beneficios del Karate</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {MOCK_BENEFITS.map((benefit) => (
            <div key={benefit.id} className="p-6 rounded-2xl flex flex-col space-y-4 glass-card-hover border border-white/5 relative overflow-hidden group shadow-md shadow-black/40" style={{ animation: "border-color-change 8s infinite linear" }}>
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center border ${benefit.colorClass}`}>{getBenefitIcon(benefit.iconName)}</div>
              <div className="space-y-2">
                <h3 className="font-bold text-xl font-display text-gray-700 group-hover:text-brand-accent transition-colors">{benefit.title}</h3>
                <p className="text-sm text-gray-700 leading-relaxed font-sans">{benefit.description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="md:py-20 py-15 max-w-7xl mx-auto px-8 md:px-[50px] space-y-5 md:space-y-12">
        <div className="text-left space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold font-display tracking-tight text-gray-700">Nuestros Instructores</h2>
          <p className="text-sm text-left sm:text-base text-gray-700/60 max-w-2xl">Aprende de cinturones negros de alto rango, con certificación oficial internacional de la organización Inoue Ha de Japón.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-2">
          {MOCK_INSTRUCTORS.slice(0, 3).map((inst) => (
            <div key={inst.id} className="rounded-2xl overflow-hidden border border-white/5 flex flex-col relative group h-full ring-1 ring-brand-accent/50 shadow-md shadow-black/40">
              <div className="h-64 sm:h-[30rem] w-full relative overflow-hidden bg-slate-900 border-b border-white/10">
                <img src={inst.imageUrl} alt={inst.name} className="w-full h-full object-cover transition-transform duration-750 group-hover:scale-[1.03]" style={{ objectPosition: inst.imagePosition ?? 'center 45%' }} />
              ...
              <div className="p-6 space-y-4 text-left flex-1 flex flex-col">
                <h4 className="font-extrabold text-2xl text-gray-700 font-display uppercase tracking-tight group-hover:text-brand-accent transition-colors whitespace-pre-line">{inst.name}</h4>
                <p className="text-sm text-gray-700 font-sans leading-relaxed">{inst.bio}</p>
                {inst.curriculum && (
                  <ul className="bg-white/5 rounded-lg border border-white/5 mt-1">
                    {inst.curriculum.slice(0, 2).map((item, idx) => (
                      <li key={idx} className="text-xs text-gray-700 flex items-start gap-1">
                        <span className="text-brand-accent mt-0.5 shrink-0">•</span><span>{item}</span>
                      </li>
                    ))}
                  </ul>
                )}
      ...
      {/* Galería */}
      <section className="md:py-20 max-w-7xl mx-auto px-8 md:px-[50px] space-y-12">
        <div className="text-left space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold font-display tracking-tight text-gray-700">Algunos Momentos</h2>
        </div>
        <GalleryLightbox />
      </section>
      ...
      <section className="px-8 py-14 md:py-16 border-t border-white/5">
        <div className="max-w-4xl mx-auto space-y-8 md:space-y-12 text-left">
          <div className="text-left space-y-3">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-left font-display uppercase tracking-tight text-gray-700">Preguntas Frecuentes</h2>
          </div>
