  ...
  return (
    <div>
      ...
      <div
        ...
        onDrop={handleDrop}
        className={`flex flex-col items-center justify-center gap-1 border-2 border-dashed rounded-lg p-4 cursor-pointer transition text-center ${
          dragging ? 'border-brand-accent bg-brand-accent/10' : error ? 'border-red-400 bg-red-50' : 'border-brand-accent/50 hover:border-brand-accent hover:bg-brand-accent/5'
        }`}
      >
        {preview ? (
...
const ToseiGusokuForm = ({ onNavigateToHome }: { onNavigateToHome: () => void }) => {
  ...
  const handleHijoChange = (id: string, field: keyof Hijo, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (validateStep3()) {
      // Aquí se enviaría a Google Forms (adaptar según los nuevos campos)
  ...
  const renderStep2 = () => {
    ...
    const sujeto = esAdulto ? 'tú' : 'tu hijo(a)';
    const posesivo = esAdulto ? 'tuyo' : 'de tu hijo(a)';
